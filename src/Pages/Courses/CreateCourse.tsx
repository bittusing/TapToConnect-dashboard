...
export default function CreateCourse() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await API.postAuthAPI(payload, END_POINT.COURSES, true);
      ...
      toast.error(error?.message || error?.error || "Failed to create course. Please check all required fields.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8">
          <h1 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Create New Course</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* <SelectGroupAntd
                label="Technology"
                selectedOption={formData.technology}
                setSelectedOption={(value) => setFormData((prev) => ({ ...prev, technology: value }))}
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    {formData.requirements.filter((item) => item.trim() !== "").map((item, index) => {
                      const actualIndex = formData.requirements.indexOf(item);
                      return (
                        <div key={index} className="flex items-center gap-2">
                          <span className="flex-1 text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 px-3 py-2 rounded">
                            {item}
                          ...
                          <AntdButton
                            ...
                            danger
                            icon={<DeleteOutlined />}
                            onClick={() => handleRemoveRequirement(actualIndex)}
                          />
                        </div>
                      );
                    })}
